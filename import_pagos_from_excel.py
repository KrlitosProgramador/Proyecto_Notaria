...
def insert_rows(supabase, rows, batch_size=100):
    ...
    for start in range(0, len(rows), batch_size):
        ...
        try:
            result = supabase.table("pagos_2026").insert(batch).execute()
        except Exception as exc:
            raise RuntimeError(
                "Error al insertar datos en Supabase. Verifica que la tabla pagos_2026 exista y que la clave usada tenga permisos de inserción."
            ) from exc
...
def main():
    parser = argparse.ArgumentParser(description="Importa Informe.xlsx a la tabla Supabase pagos_2026.")
    parser.add_argument(
        "--excel",
        default="pagos_2026.xlsx",
        help="Ruta al archivo Excel que contiene los registros (por defecto: pagos_2026.xlsx)",
    )
    parser.add_argument(
        "--batch",
        type=int,
